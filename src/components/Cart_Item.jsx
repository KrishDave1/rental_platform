...
export default function CartItem({ item }) {
  const { handleDelete, handleAdd, Delete, cartItems, handleAddWish,  totalPrice,setTotalPrice,months,setMonths } =
    useGlobalContext();
        ...
        <div className="flex items-center w-full justify-around">
          ...
          <div className="flex flex-col justify-center">
            <div className="m-3">
              ...
              {item.Percentage_off==="undefined" || item.Price_was==="undefined"? "M.R.P: "+"₹"+(PriceRs.toFixed(2)*cartItems[item.id]).toFixed(2)+"/mo"+" (0% off)": "M.R.P: ₹"+(PriceWasRs.toFixed(2)*cartItems[item.id]).toFixed(2)+"/mo"+` (${item.Percentage_off}% off)`}
              </div>
              <p className="text-lg font-semibold">Our Price: ₹{(PriceRs*cartItems[item.id]).toFixed(2)}/mo</p>
            </div>
            <div>
                Total Price for {months} months: ₹ {(PriceRs.toFixed(2)*months*cartItems[item.id]).toFixed(2)}
